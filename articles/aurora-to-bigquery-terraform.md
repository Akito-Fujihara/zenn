...
1. **Aurora を直接サポート**: Datastream は Aurora に対応していない（MySQL のみ）
2. **ネットワーク構成がシンプル**: VPN / Interconnect 不要でインターネット経由の転送が可能
3. **Parquet 形式**: カラム型フォーマットで BigQuery の分析クエリに最適化
4. **セキュアなクロスクラウド認証**: OpenID Connect フェデレーションでアクセスキー不要
5. **疎結合**: 各コンポーネントが独立しており、障害の影響範囲が限定的
